 ...
 */
namespace S3FileManager\Model\Behavior;
...
class UploadableBehavior extends Behavior
{
    ...
    public function __construct(Table $table, array $config = [])
    {
        ...
        Type::map('S3FileManager.File', 'S3FileManager\Database\Type\FileType');

        $schema = $table->schema();
        foreach ($this->getFieldList() as $field => $settings) {
            $schema->columnType($field, 'S3FileManager.File');
        }
        $table->schema($schema);
    ...
    public function beforeSave($event, $entity, $options)
    {
        ...
        if (!isset($options['loggedInCustomer'])) {
            $options['loggedInCustomer'] = '';
        }
